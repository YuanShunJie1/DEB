...
def euclidean_dist(pointA, pointB):
    """
    计算欧氏距离的平方，兼容 numpy.matrix 和 ndarray
    """
    diff = np.asarray(pointA) - np.asarray(pointB)  # 转为 ndarray
    return np.sum(diff ** 2)    

def gaussian_kernel(distance, bandwidth):
    m = np.shape(distance)[0]  
    right = np.asmatrix(np.zeros((m, 1)))  
    for i in range(m):
...
def train_mean_shift(points, kenel_bandwidth=2):
    mean_shift_points = np.asmatrix(points)
    max_min_dist = 1
    iteration = 0  
...
def search_vec(center,target_clean_vecs):
    ...
    target_vec = center * max_length/torch.norm(center,p=2)*5.0
